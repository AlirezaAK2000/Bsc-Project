...
from agents.common.networks import SACCriticAtari, SACActorAtari
import copy
...
class ReplayBuffer:
    ...
    def __init__(self, buffer_size, batch_size, num_classes, device):
    ...
    def sample(self):
        ...
        states = torch.from_numpy(np.stack([e.state.squeeze() for e in experiences if e is not None])).float().to(self.device) / self.num_classes
        actions = torch.from_numpy(np.vstack([e.action for e in experiences if e is not None])).float().to(self.device)
        rewards = torch.from_numpy(np.vstack([e.reward for e in experiences if e is not None])).float().to(self.device)
        next_states = torch.from_numpy(np.stack([e.next_state.squeeze() for e in experiences if e is not None])).float().to(
...
class SAC(nn.Module):
    ...
    def __init__(self,
        ...
        self.actor_local = SACActorAtari(action_size, num_frames=state_size[0]).to(device)
        self.actor_optimizer = optim.Adam(self.actor_local.parameters(), lr=learning_rate)

        # Critic Network (w/ Target Network)

        self.critic1 = SACCriticAtari(action_size, state_size[0]).to(device)
        self.critic2 = SACCriticAtari(action_size, state_size[0]).to(device)

        assert self.critic1.parameters() != self.critic2.parameters()

        self.critic1_target = SACCriticAtari(action_size, state_size[0]).to(device)
        self.critic1_target.load_state_dict(self.critic1.state_dict())

        self.critic2_target = SACCriticAtari(action_size, state_size[0]).to(device)
